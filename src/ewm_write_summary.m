...
if isfield(results, 'referenceComparison') && ~isempty(results.referenceComparison)
    fprintf(fid, '\n');
    fprintf(fid, '实验3参考解间距_m = %.15g\n', ...
        results.referenceComparison.referenceSpacing);
    if isfield(results.referenceComparison, 'referenceSource')
        fprintf(fid, '实验3参考解来源 = %s\n', ...
            results.referenceComparison.referenceSource);
    end
    fprintf(fid, '实验3_Taylor相对参考波场偏离_平均 = %.15g\n', ...
        results.referenceComparison.standardMeanReferenceError);
    fprintf(fid, '实验3_优化系数相对参考波场偏离_平均 = %.15g\n', ...
        results.referenceComparison.minimaxMeanReferenceError);
    if isfield(results.referenceComparison, 'standardMeanHighKRatio')
        fprintf(fid, '实验3_Taylor高波数能量占比_平均 = %.15g\n', ...
            results.referenceComparison.standardMeanHighKRatio);
        fprintf(fid, '实验3_优化系数高波数能量占比_平均 = %.15g\n', ...
            results.referenceComparison.minimaxMeanHighKRatio);
    end
end

if isfield(results, 'boundaryReferenceComparison') && ~isempty(results.boundaryReferenceComparison)
    fprintf(fid, '\n');
    fprintf(fid, '实验2扩大计算域每侧扩展网格点数 = %d\n', ...
        results.boundaryReferenceComparison.extraGrid);
    fprintf(fid, '实验2无吸收_相对参考波场偏离_平均 = %.15g\n', ...
        results.boundaryReferenceComparison.noAbsorbMeanReferenceError);
    fprintf(fid, '实验2_PML_相对参考波场偏离_平均 = %.15g\n', ...
        results.boundaryReferenceComparison.pmlMeanReferenceError);
    fprintf(fid, '实验2无吸收_边界带波场偏离_平均 = %.15g\n', ...
        results.boundaryReferenceComparison.noAbsorbMeanBoundaryReferenceError);
    fprintf(fid, '实验2_PML_边界带波场偏离_平均 = %.15g\n', ...
        results.boundaryReferenceComparison.pmlMeanBoundaryReferenceError);
    fprintf(fid, '实验2_PML全域偏离降低百分比 = %.15g\n', ...
        100 * (results.boundaryReferenceComparison.noAbsorbMeanReferenceError - ...
        results.boundaryReferenceComparison.pmlMeanReferenceError) / ...
        (results.boundaryReferenceComparison.noAbsorbMeanReferenceError + eps));
    fprintf(fid, '实验2_PML边界带偏离降低百分比 = %.15g\n', ...
        100 * (results.boundaryReferenceComparison.noAbsorbMeanBoundaryReferenceError - ...
        results.boundaryReferenceComparison.pmlMeanBoundaryReferenceError) / ...
        (results.boundaryReferenceComparison.noAbsorbMeanBoundaryReferenceError + eps));
end
...
fprintf(fid, '   summary 中其他形如"相对参考波场偏离""高波数能量占比""边界带波场偏离"的量是端到端波场可视化的辅助诊断量，不属于"误差"范畴。\n');
fprintf(fid, '   相对参考波场偏离的计算方式：先以最小二乘估计幅值因子 alpha = <u, uref> / <uref, uref>，再取 ||u - alpha·uref||_2 / ||alpha·uref||_2，目的是抵消整体振幅尺度差异。\n');
fprintf(fid, '   高波数能量占比定义为二维径向波数大于 0.55 倍奈奎斯特波数的谱能量占总谱能量的比例。\n');
fprintf(fid, '\n');
